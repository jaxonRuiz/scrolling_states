...
class IdleState extends State {
    ...
    execute(scene, hero) {
        ...
        const FKey = scene.keys.FKey; // only grabable because defined in Play.js
        ...
        // circular attack if F key pressed
...
class SpinState extends State {
    enter(scene, hero) {
        //hero.setTint(0x0000FF); // tints hero blue
        // tinting only works with PHaser.WEBGL in game config
        hero.setVelocity(0);
        hero.play('swing-360').once('animationcomplete', ()=> {
            this.stateMachine.transition('idle');
        })
        scene.cameras.main.shake(1500, 1); // adding camera shake VERY COOL AND FUN :D
    }
    // execute(hero, scene) {
    //     if (Phaser.Input.Keyboard.JustDown(FKey)) {
    //         this.stateMachine.transition('spin');
